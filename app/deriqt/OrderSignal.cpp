...
OrderData* OrderSignal::get_signal(const std::vector<std::string>&v_rev, int trading_mode, std::vector<Position *> v_positions,StrategyConfig *p_strategy_config, int order_limit){
	...
	if(_is_long){ //get long signal
		...
		for(auto it=v_positions.begin(); it!=v_positions.end();++it){
			...
			if(p_curr_pos->side == PositionSide_Long){
				...
				LOG(INFO)<<"Check close long,last_price:"<<_last_price<<",vwap:"<<p_curr_pos->vwap<<",stop_profit:"<<p_strategy_config->stop_profit;
