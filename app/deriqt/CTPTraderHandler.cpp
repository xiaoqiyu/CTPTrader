...
void CTPTraderHandler::processRspUserLogin(Task* task)
{
	if (task->task_data)
	{
        ...
        LOG(INFO)<<"Login Success,login_time=>"<<task_data->LoginTime<<",涓婃湡鎵�=>"<<task_data->SHFETime<<",澶у晢鎵�=>"<<task_data->DCETime<<"閮戝晢鎵�=>"<<task_data->CZCETime<<",涓噾鎵�=>"<<task_data->FFEXTime<<",涓婃湡鑳芥簮=>"<<task_data->INETime<<",local time=>"<<ltm->tm_min<<":"<<ltm->tm_sec;
...
{
	//ProductClass:,"1":鏈熻揣鍗曚竴鍚堢害锛?"2":鏈熸潈鍗曚竴鍚堢害
...
void CTPTraderHandler::processRtnOrder(Task* task)
{
	if (task->task_data)
	{
        ...
        // _login_time 是登陆时返回的交易所时间，task_data inserttime也是交易所返回时间
        if(!(strcmp(task_data->InsertTime, _login_time) > 0 && _is_same_date )){
            ...
            order_available_ = true; //忽略订单之后释放订单可查询锁
            cond_.notify_all();
            return;
        }
        ...
        //the 2nd order callback, when the order is accepted by the exchange
            ...
            p_orderfield->VolumeTotal = task_data->VolumeTotal;
            p_orderfield->OrderStatus = task_data->OrderStatus;
            // p_orderfield->InsertTime = now_time; //第二个callback 不修改委托时间
            if(p_orderfield->OrderStatus == THOST_FTDC_OST_Canceled){ //TODO check whether there is other status to handle here, if cancel,the order is removed, and not processed
                LOG(INFO)<<"[update order table] remove order when canceled with order_id1=>"<<_order_id1; //撤单的话释放订单查询锁，还有委托限制锁
        ...
        //the 1st order callback, when the order is accepted by CTP, but not yet submit to the exchagne
        ...
        LOG(INFO)<<"OrderStatus [0:all traded,1:partial traded-in queue,3:in queue,5:canceled,a:unknown] =>"<<task_data->OrderStatus;
        ...
        if(trade_product && task_data->OrderStatus == THOST_FTDC_OST_Canceled){//如果是交易的品种，而且订单撤销，释放订单锁
            order_complete_ = true;
	...
	if (task->task_data)
	{
        ...
        if(it != m_all_orders.end()){ //update order field
            ...
            if((p_orderfield->VolumeTraded == p_orderfield->VolumeTotalOriginal && p_orderfield->OrderStatus==THOST_FTDC_OST_AllTraded)){ 
                ...
                order_complete_ = true;
                cond_.notify_all();
                LOG(INFO)<<"[processRtnTrade] order completed  with order_id=>"<<it->first<<",status [0,tradded,5:canceled] =>"<<p_orderfield->OrderStatus<<", order_complette=>"<<order_complete_;
            }
        }
...
void CTPTraderHandler::init(const std::string& task_tag, const std::string& broker_id, const std::string& user_id)
{
	...
	cond_.wait(mlock, [&]() {
		return connected_; //璋冪敤浜唎nfrontconnected鏃跺�欐墠浼氬皢ready_璁剧疆涓簍rue锛屾病鏈夎皟鐢╫nfrontconnected,灏变細涓�鐩磜ait.
	}); 
};
...
int CTPTraderHandler::insert_order(OrderData * p_orderdata){
    ...
    //TODO double check 涓嬪崟绫诲瀷锛泃est SHFE,鍙湁鍗虫垚鎾ゅ崟锛屾垨鑰呭綋鏃ユ湁鏁堝彲鐢?    
    // orderfield.TimeCondition = THOST_FTDC_TC_IOC;//绔嬪嵆瀹屾垚锛屽惁鍒欐挙鍗?
    orderfield.TimeCondition = THOST_FTDC_TC_GFD;//褰撴棩鏈夋晥
...
void CTPTraderHandler::update_positions(CThostFtdcTradeField* p_trade){
    ...
    //濡傛灉鏄钩浠撱�佸钩浠婃垨鑰呭钩鏄紝鍒欎氦鏄撴暟閲忓彉鍔ㄦ槸璐熺殑
...
{
    unique_lock<mutex> mlock(mutex_);
    // std::cout<<"wait for order available to return"<<std::endl;
    cond_.wait(mlock, [&]() {
